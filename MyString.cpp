...
MyString operator+(const MyString& secondVal, const MyString& firstVal)
{
    MyString newStr;
    newStr.length_ = secondVal.length_ + firstVal.length_;
    newStr.str_ = new char[newStr.length_ + 1];
    int i = 0;
    for (; i < secondVal.length_; i++)
    {
        newStr.str_[i] = secondVal.str_[i];
    }
    for (int j = 0; j < firstVal.length_; j++, i++) 
    {
        newStr.str_[i] = firstVal.str_[j];
    }
    newStr.str_[secondVal.length_ + firstVal.length_] = '\0';
    return newStr;
}

MyString operator+(const MyString& secondVal, const char* firstVal)
{
    MyString newStr;
    newStr.length_ = secondVal.length_ + strLength(firstVal);
    newStr.str_ = new char[newStr.length_ + 1];
    int i = 0;
    for (; i < secondVal.length_; i++) 
    {
    ...
    for (int j = 0; j < strLength(firstVal); j++, i++) 
    {
        newStr.str_[i] = firstVal[j];
    }
    newStr.str_[secondVal.length_ + strLength(firstVal)] = '\0';
    return newStr;
}

MyString operator+(const char* secondVal, const MyString& firstVal)
{
    MyString newStr;
    newStr.length_ = strLength(secondVal) + firstVal.length_;
    newStr.str_ = new char[newStr.length_ + 1];
    int i = 0;
    for (; i < strLength(secondVal); i++) 
    {
        newStr.str_[i] = secondVal[i];
    }
    for (int j = 0; j < firstVal.length_; j++, i++) 
    {
        newStr.str_[i] = firstVal.str_[j];
    }
    newStr.str_[strLength(secondVal) + firstVal.length_] = '\0';
    return newStr;
}
...
char* MyString::rbegin() noexcept
{ 
    return end();
}

char* MyString::rend() noexcept 
{ 
    return begin(); 
}
